...
set ref_array = array(select distinct f_mes_acc_carga from `meli-bi-data.SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_BACKTEST_BAU_PECASTANHO` order by 1 asc);
...
drop table if exists `meli-bi-data.SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_BACKTEST_BAU_PL_MOB_PECASTANHO`;

create or replace table `meli-bi-data.SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_BACKTEST_BAU_PL_MOB_PECASTANHO`
(
  user_id INT64
...
temp00 as (
  ...
  from `meli-bi-data.SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_BACKTEST_BAU_PECASTANHO`
  where 1=1
...
insert into `meli-bi-data.SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_BACKTEST_BAU_PL_MOB_PECASTANHO` (
  user_id
  ,f_mes_acc_carga
  ,f_acc_carga
  ,f_acc_impacto
