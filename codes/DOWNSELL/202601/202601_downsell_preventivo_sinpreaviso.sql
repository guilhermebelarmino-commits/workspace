-- SELECT * FROM `meli-bi-data.SBOX_CREDITSSIGMA.CAMP_EOC_RK_fd2ae13c-59d8-4fd8-9b26-033f8205a3f9-1`;SELECT POL.*, RK.eliminated_by_rk FROM `meli-bi-data.SBOX_CREDITSSIGMA.CAMP_EOC_RK_POLICY_fd2ae13c-59d8-4fd8-9b26-033f8205a3f9-1` AS POL INNER JOIN `meli-bi-data.SBOX_CREDITSSIGMA.CAMP_EOC_RK_fd2ae13c-59d8-4fd8-9b26-033f8205a3f9-1` AS RK ON POL.CUS_CUST_ID = RK.CUS_CUST_ID;SELECT * FROM `meli-bi-data.SBOX_CREDITSSIGMA.EOC_CAMPAIGN_EXECUTION_DETAIL` WHERE EXECUTION_ID = 'fd2ae13c-59d8-4fd8-9b26-033f8205a3f9';

DECLARE DT_REF DATE;
SET DT_REF = '2026-01-20'; -- D-1
...
CREATE OR REPLACE TEMP TABLE SIMULACION AS
  ...
  FROM `meli-bi-data.SBOX_CREDITSSIGMA.CAMP_EOC_RK_fd2ae13c-59d8-4fd8-9b26-033f8205a3f9-1` RK
  LEFT JOIN `meli-bi-data.SBOX_CREDITSSIGMA.CAMP_EOC_RK_POLICY_fd2ae13c-59d8-4fd8-9b26-033f8205a3f9-1` POL
  USING(cus_cust_id)
;
...
CREATE OR REPLACE TEMP TABLE CDP AS 
  ...
  where 1=1
  and SUMMARY_PERIOD IN ('202512') -- M-1
  and sit_site_id = 'MLB'
...
CREATE OR REPLACE TABLE `SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_SIN_PREAVISO_202601_GUBELARMINO` AS
...
FROM (
  SELECT
    *,
    CASE 
      WHEN TIPO_PAGO = 'A. Transactor' THEN 0.8*limit_amount_tc
      ELSE
        CASE  
    ...
    case
      when bucket_vencimiento in (2, 5, 9, 18, 27, 30) OR bucket_vencimiento IS NULL THEN 'a. out'
      when bucket_vencimiento in (12, 15) THEN 'b. in'
    END AS FLAG_BUCKET,
    CASE 
  ...
  FROM (
    SELECT
      *
      ,CASE
        WHEN IFNULL(SCR_DATA_BASE_DT, '1900-01-01') < DATE_SUB(DT_REF, INTERVAL 6 MONTH) THEN 'z. info scr desactualizada'
      ...
      SELECT
        *
        ,CASE
          ...
          -- deterioro deuda clean 
          ,CASE
            WHEN DEUDA_CLEAN_30D < 300 OR IFNULL(SCR_DATA_BASE_DT, '1900-01-01') < DATE_SUB(DT_REF, INTERVAL 6 MONTH) OR IFNULL(SCR_DATA_BASE_DT_M3, '1900-01-01') < DATE_SUB(DT_REF, INTERVAL 6 MONTH) THEN 'z. sem info scr'
          ...
          ,CASE
            WHEN WANDA__PERSON_LAST_CCARD_CONSUMPTION_STMT_STATUS IN ('ADVANCE PAYMENT', 'TOTAL PAYMENT') THEN 'A. Transactor'
            WHEN WANDA__PERSON_LAST_CCARD_CONSUMPTION_STMT_STATUS IN ('PARCELED PAYMENT', 'PARTIAL PAYMENT', 'MINIMUM PAYMENT') THEN 'B. Revolving'
            WHEN WANDA__PERSON_LAST_CCARD_CONSUMPTION_STMT_STATUS IN ('DEFAULTED') THEN 'C. Defaulted'
            ELSE 'Z. outros'
          END AS TIPO_PAGO,
      FROM
      (
        SELECT
          *
          ...
          ,IF(IFNULL(DEUDA_VENCIDA_OVER30,0) > 100, 1, 0) AS FL_DEUDA_VENC_OVER30_CLEAN -- deuda vencida
...
CREATE OR REPLACE TABLE `SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_SIN_PREAVISO_FINAL_202601_GUBELARMINO` AS

select 
  *
from `SBOX_CREDITS_SB.RBA_TC_MLB_DOWNSELL_SIN_PREAVISO_202601_GUBELARMINO`
where 1=1
